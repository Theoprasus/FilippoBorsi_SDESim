...
namespace LLN
{
    public partial class FormGeometricBrownian : Form
    {
        public FormGeometricBrownian()
        {
            InitializeComponent();
            this.textBoxM.Text = "5000";
            this.textBoxN.Text = "1000";
            this.textBoxMu.Text = "0.15";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //limited 0,n
            if (ui.getDouble(this.textBoxSigma.Text, 0, 1) != null)
            ...
            double Mu = 0.15;
            if (!string.IsNullOrEmpty(ui.getDouble(this.textBoxMu.Text, -n, n).ToString()) )
                Mu = (double)ui.getDouble(this.textBoxMu.Text, -n, n);



            this.textBoxMu.Text = Mu.ToString();
            ...
            List<List<PointF>> listLinesRandom = getListOfBrownian(m, n, sigma, Mu, starting_value);
        ...
        public List<List<PointF>> getListOfBrownian(int m, int n, double sigma, double mu, double starting_value)
        {
            ...
            for (int index_listaLinee = 0; index_listaLinee < m; index_listaLinee++)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    Y_past += GeometricBrownianupdate(sigma, ranGenerator, n, mu, starting_value, i, Y_past );
        ...
        private double GeometricBrownianupdate(double sigma, Random rangen, int n_tot, double mu, double starting_value, int t, double step_precedente)
        {
            ...
            double dx = mu * step_precedente * dt + sigma * step_precedente * Math.Sqrt(dt) * generateGaussian(0, 1, rangen);
            return dx;
